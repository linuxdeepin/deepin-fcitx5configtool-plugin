...
void KeyboardLayoutWidget::setGroup(int group) {
    ...
    rdefs.model = !model.isNull() ? strdup(model.toUtf8().constData()) : nullptr;
    ...
    rdefs.variant = variants.count() > group
                        ...
                        : nullptr;
    rdefs.options =
        !option.isNull() ? strdup(option.toUtf8().constData()) : nullptr;
...
void KeyboardLayoutWidget::setKeyboardLayout(const QString &layout,
    ...
    rdefs.model = !model.isNull() ? strdup(model.toUtf8().constData()) : nullptr;
    rdefs.layout =
        !layout.isNull() ? strdup(layout.toUtf8().constData()) : nullptr;
    rdefs.variant =
        !variant.isNull() ? strdup(variant.toUtf8().constData()) : nullptr;
    rdefs.options =
        !option.isNull() ? strdup(option.toUtf8().constData()) : nullptr;
...
void KeyboardLayoutWidget::init() {
    ...
    for (i = 0; i < xkb->geom->num_sections; i++) {
        ...
        for (j = 0; j < section->num_rows; j++) {
            ...
            //虚拟键盘位置
            x = section->left + 650 + row->left;
            y = section->top - 200 + row->top;
...
void KeyboardLayoutWidget::initFilterKeys()
{
    //ESC - F12
    m_filterkeys << 9 << 67 << 68 << 69 << 70 << 71 << 72 << 73 << 74 << 75 << 76 << 95 << 96;
    //PrtSc - PGDN
    m_filterkeys << 107 << 78 << 127 << 118 << 110 << 112 << 119 << 115 << 117;
    //UP - RGHT
    m_filterkeys << 111 << 113 << 114 << 116;
    //小键盘区
    m_filterkeys << 77 << 106 << 63 << 82 << 79 << 80 << 81 << 86 << 83 << 84 << 85 << 87 << 88 << 89 << 104 << 90 << 91 << 125;
}
...
//        level = strtol(colorspec + 4, nullptr, 10);
...
//        color.setBlueF(1.0 - level / 100.0);
        ...
        level = strtol(colorspec + 3, nullptr, 10);

        color.setRedF(level / 100.0);
        color.setGreenF(0);
        color.setBlueF(0);
    } else if (strncasecmp(colorspec, "green", 5) == 0) {
//        level = strtol(colorspec + 5, nullptr, 10);
...
void KeyboardLayoutWidget::initInicatorDoodad(XkbDoodadRec *xkbdoodad,
    ...
    if (!XkbGetNamedIndicator(QX11Info::display(), sname, nullptr, &doodad.on,
                              nullptr, NULL))
        doodad.on = 0;
}
...
void KeyboardLayoutWidget::generatePixmap(bool force) {
    ...
    double ratioX = (double)width() / xkb->geom->width_mm * 1.62;
    double ratioY = (double)height() / xkb->geom->height_mm * 1.62;

//    double ratioX = (double)width() / xkb->geom->width_mm * 3;
//    double ratioY = (double)height() / xkb->geom->height_mm * 3;
...
{
    if(m_filterkeys.contains(key->keycode)) {
        return;
    }
...
//    else
        color = "#c2d3ee"; //colors[key->xkbkey->color_ndx];
    ...
    for (i = 0; i < shape->num_outlines; i++) {
        if (shape->outlines + i == shape->approx ||
            ...
            continue;
        draw_outline (context, shape->outlines + i, nullptr,
...
void KeyboardLayoutWidget::roundedCorner(QPainterPath &path, QPointF b,
    ...
    QRectF arcRect(ctr.x() - radius, ctr.y() - radius, 4 * radius - 2, 4 * radius - 4);
...
void KeyboardLayoutWidget::roundedPolygon(QPainter *painter, bool filled,
    ...
    path.closeSubpath();
    painter->setPen(QColor("#C2D3EE"));
    if (filled) {
        painter->fillPath(path, QColor("#C2D3EE"));
    } else {
...
void KeyboardLayoutWidget::drawRectangle(QPainter *painter, QColor color,
    ...
    if (angle == 0) {
        ...
        //键盘布局的边框大小，
        width = xkbToPixmapCoord(xkb_x + xkb_width) - x - 3;
        height = xkbToPixmapCoord(xkb_y + xkb_height) - y - 3;
...
void KeyboardLayoutWidget::drawCurveRectangle(QPainter *painter, bool filled,
    ...
    path.arcTo(x, y, 4 * radius, 4 * radius, 180, -90);
    path.lineTo(x1 - radius, y);
    path.arcTo(x1 - 4 * radius, y, 4 * radius, 4 * radius, 90, -90);
    path.lineTo(x1, y1 - radius);
    path.arcTo(x1 - 4 * radius, y1 - 4 * radius, 4 * radius, 4 * radius, 0,
               -90);
    path.lineTo(x + radius, y1);
    path.arcTo(x, y1 - 4 * radius, 4 * radius, 4 * radius, -90, -90);
    path.closeSubpath();
...
void KeyboardLayoutWidget::drawKeyLabel(QPainter *painter, unsigned int keycode,
    ...
    for (int glp = KEYBOARD_DRAWING_POS_TOPLEFT;
        ...
        if (trackModifiers) {
            ...
            keysym = XkbKeySymEntry(xkb, keycode, 0, 0);
            syms[glp] = keySymToString(keysym);
        }
    }
